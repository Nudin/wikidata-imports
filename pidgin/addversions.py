...
statedin.setTarget("https://bitbucket.org/pidgin/www/src/tip/htdocs/ChangeLog")
title = pywikibot.Claim(repo, 'P1476')
title.setTarget(pywikibot.WbMonolingualText("ChangeLog", "en"))
retrieved = pywikibot.Claim(repo, 'P813')
date = pywikibot.WbTime(year=2017, month=11, day=5)
retrieved.setTarget(date)


for line in open('pidgin-versions'):
    [version, date] = line.split()
    # Don't add bugfix-releases for now
    if (version[-2:] != ".0"):
        continue
    item = pywikibot.ItemPage(repo, pidgin)
    item.get()
